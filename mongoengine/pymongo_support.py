...
def count_documents(
    ...
    if PYMONGO_VERSION >= (3, 7):
        try:
            is_active_session = _get_session() is not None
...
async def async_count_documents(
    ...
    for option, option_value in kwargs.items():
        ...
        cursor = cursor_method(option_value)
    with_limit_and_skip = "skip" in kwargs or "limit" in kwargs
    return await cursor.count(with_limit_and_skip=with_limit_and_skip)
...
def list_collection_names(db, include_system_collections=False):
    ...
    if PYMONGO_VERSION >= (3, 7):
        ...
        collections = db.collection_names(session=_get_session())
...
async def async_list_collection_names(db, include_system_collections=False):
    collections = await db.list_collection_names(session=_get_session())
    if not include_system_collections:
        collections = [c for c in collections if not c.startswith("system.")]
