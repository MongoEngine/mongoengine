...
def register_connection(
    ...
    :param mongo_client_class: using alternative connection client other than
        pymongo.MongoClient, e.g., mongomock, montydb, that provides pymongo similar
...
def disconnect(alias=DEFAULT_CONNECTION_NAME):
    ...
    connection: MongoClient | None = _connections.pop(alias, None)
    if connection:
...
def get_connection(alias=DEFAULT_CONNECTION_NAME, reconnect=False):
    ...
    # it immediately.
    if alias in _connections and isinstance(_connections[alias], MongoClient):
...
def get_db(alias=DEFAULT_CONNECTION_NAME, reconnect=False):
    ...
    if alias not in _dbs or not isinstance(_dbs[alias], Database):
