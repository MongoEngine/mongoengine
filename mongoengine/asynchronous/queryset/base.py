...
from mongoengine.common import _import_class, _async_queryset_to_values
...
class AsyncBaseQuerySet(abc.ABC):
    """Asynchronous version of BaseQuerySet for MongoDB queries.
    ...
    qs = User.aobjects(active=True).filter(age__gte=18)

    # Get single document (async)
    user = await User.aobjects(email='test@example.com').get()

    # Get first document or None (async)
    user = await User.aobjects(active=True).first()

    # Count documents (async)
    count = await User.aobjects(active=True).count()

    # Iterate results (async)
    ...
    # Field projection
    users = User.aobjects.only('name', 'email')
    async for user in users:
        print(user.name) # Only name and email are loaded

    # Pagination (use skip/limit instead of slicing)
    first_10 = User.aobjects.limit(10)
    next_10 = User.aobjects.skip(10).limit(10)
    ...
    def __init__(self, document: typing.Type['Document']):
        ...
        self._mongo_query = None  # Cached MongoDB query dict
        self._query_obj = Q()  # MongoEngine query object
        self._cls_query = {}  # Query filter for inheritance (_cls field)
        self._where_clause = None  # JavaScript $where clause
        self._loaded_fields = QueryFieldList()  # Fields to load (projection)
        self._ordering = None  # Sort order for results
        self._snapshot = False  # Deprecated snapshot mode
        self._timeout = True  # Enable MongoDB cursor timeout
        self._allow_disk_use = False  # Allow disk usage for large sorts
        self._read_preference = None  # MongoDB read preference
        self._read_concern = None  # MongoDB read concern
        self._iter = False  # Iteration state flag
        self._scalar = []  # Fields for scalar/values_list mode
        self._none = False  # Return empty results without querying DB
        self._using: tuple[str, str] | None = None
        self._as_pymongo = False  # Return raw pymongo dicts instead of Documents
        self._search_text = None  # Text search query
        self._search_text_score = None  # Include text search scores

        # Async-specific: Lock to prevent multiple concurrent awaits on a collection
    ...
    def __getitem__(self, key: Union[int, slice]):
        ...
        """
        # Both slicing and integer indexing are disabled in async version
        if isinstance(key, slice):
            raise OperationError(
                "AsyncQuerySet does not support slicing (qs[start:stop]). "
                "Use method chaining instead:\n"
                "  • qs.limit(n) instead of qs[:n]\n"
                "  • qs.skip(n).limit(m) instead of qs[n:n+m]\n"
                "  • qs.skip(n) instead of qs[n:]"
            )
        elif isinstance(key, int):
            raise OperationError(
                "AsyncQuerySet does not support indexing (qs[n]). "
    ...
    def __bool__(self):
        """Disabled in async context to prevent accidental synchronous evaluation.

        Raises:
            TypeError: Always, with instructions on proper async usage

        Note:
            In sync BaseQuerySet, bool(qs) fetches data synchronously.
            This is impossible in an async context, so use `await qs.exists()` instead.
        """
        raise TypeError(
            "AsyncQuerySet cannot be used in a boolean context. "
            "Use 'if await qs.exists()' or 'if (await qs.first()) is not None'."
        )

    async def exists(self):
        """Efficiently check if any documents match the query.

        Async equivalent of sync BaseQuerySet's __bool__ method.
        Performs an optimized query to check existence without fetching all data.

        Returns:
            bool: True if at least one matching document exists

        Example:
            if await User.aobjects(active=True).exists():
                print("Active users found")
        """
        qs = self.clone()

        # Ignore ordering entirely (sync behavior)
        qs._ordering = []
    ...
    async def get(self, *q_objs, **query):
        ...
        Async version of BaseQuerySet.get(). Efficiently checks for
    ...
    async def create(self, **kwargs):
        ...
        """
        return await self._document(**kwargs).asave(force_insert=True)

    async def first(self):
        """Retrieve the first document matching the query.

        Async version of BaseQuerySet.first(). Returns None if no matches found.
    ...
    async def insert(
        ...
        Async version of BaseQuerySet.insert(). Supports single or multiple
        ...
        Args:
            ...
            write_concern: MongoDB write concern options (e.g., {w: 2, fsync: True})
        ...
        if isinstance(docs, Document) or issubclass(docs.__class__, Document):
            return_one = True
            docs = [docs]
        elif isinstance(docs, AsyncQuerySet):
            docs = [doc async for doc in docs]
        for doc in docs:
        ...
        for doc, doc_id in zip(docs, ids):
            doc.pk = doc_id
        if not load_bulk:
            await signals.post_bulk_insert.send_async(
    ...
    async def delete(self, write_concern=None, _from_doc_delete=False, cascade_refs=None):
        ...
        if call_document_delete:
            ...
            async for doc in queryset:
                await doc.adelete(**write_concern)
                cnt += 1
            return cnt
        ...
        for rule_entry, rule in delete_rules:
            ...
            if rule == DENY:
                refs = document_cls.aobjects(**{field_name + "__in": self})
        ...
        with set_write_concern(await queryset._collection, write_concern) as collection:
            result = await collection.delete_many(
                await _async_queryset_to_values(queryset._query),
                session=_get_session(),
                **kwargs,
            )
    ...
    async def update(
        ...
        Args:
            upsert: Insert document if no match exists
        ...
        Raises:
            NotUniqueError: If update causes duplicate key violation
            OperationError: If update fails or no update params provided

        Example:
            # Simple update
            count = await User.aobjects(active=False).update(set__active=True)

            # Increment field
            await Post.aobjects(id=post_id).update(inc__views=1)

            # Array operations
            await User.aobjects(id=uid).update(push__tags='python')
        """
        ...
        try:
            with set_read_write_concern(
                ...
                result = await update_func(
                    query,
                    await _async_queryset_to_values(update),
                    upsert=upsert,
    ...
    async def upsert_one(self, write_concern=None, read_concern=None, **update):
        ...
        if atomic_update.raw_result["updatedExisting"]:
            ...
            document = await self._document.aobjects.with_id(atomic_update.upserted_id)
        return document
    ...
    async def modify(
        ...
        try:
            if remove:
                result = await (await queryset._collection).find_one_and_delete(
                ...
                result = await (await queryset._collection).find_one_and_update(
                    query,
                    await _async_queryset_to_values(update),
                    upsert=upsert,
                    sort=sort,
    ...
    async def in_bulk(self, object_ids):
        ...
        Example:
            ...
            users_dict = await User.aobjects.in_bulk(user_ids)
        ...
        collection = await self._collection  # this part *is* awaitable
    ...
    def using(self, alias: str | None = None, collection_name: str = None):
    ...
    def _clone_into(self, new_qs):
        if not isinstance(new_qs, AsyncBaseQuerySet):
            raise OperationError(
    ...
    async def distinct(self, field):
        ...
        if not needs_aggregation(queryset):
            cursor = await queryset._cursor
            raw_values = await cursor.distinct(field)
            ...
            # EmbeddedDocument → build from SON
            if isinstance(doc_field, EmbeddedDocumentField):
            ...
            if isinstance(doc_field, ReferenceField):
                ids = raw_values
                objs = await doc_field.document_type.aobjects.in_bulk(ids)
        ...
        if not isinstance(doc_field, ListField):
            ...
            raw = await (await coll.aggregate(pipeline)).to_list(None)
    ...
    def fields(self, _only_called=False, **kwargs):
        ...
        if _only_called:
            ...
            # If user explicitly excluded `_id`, keep it excluded
    ...
    async def explain(self):
        ...
        """
        return await (await self._cursor).explain()

    # DEPRECATED. Has no more impact on PyMongo 3+
    def snapshot(self, enabled):
        """Enable or disable snapshot mode when querying.

        :param enabled: whether snapshot mode is enabled
        """
    ...
    async def to_json(self, *args, **kwargs):
        ...
        return json_util.dumps([a async for a in self.as_pymongo()], *args, **kwargs)
    ...
    async def aggregate(self, pipeline, **kwargs):
        ...
        Returns:
            AsyncIOMotorCursor: Async cursor over aggregation results

        Raises:
            TypeError: If the pipeline is not a list or tuple
        ...
        Example:
            ...
            cursor = await User.aobjects(active=True).aggregate(pipeline)
            async for result in cursor:
                print(result)

        Note:
            geoNear and collStats must be first in pipeline if used
        """
    ...
    async def map_reduce(
        ...
        if isinstance(output, dict) and "db_alias" in output:
            output_db = await async_get_db(output["db_alias"])
        else:
            output_db = await queryset._document._async_get_db()
        ...
        source_db = await queryset._document._async_get_db()

        result = await source_db.command(
            {
    ...
    async def exec_js(self, code, *fields, **options):
        ...
        db = await queryset._document._async_get_db()
        return await (await db.command("eval", code, args=fields)).get("retval")
    ...
    async def sum(self, field):
        """Calculate sum of values for a field across matching documents.
    ...
    async def average(self, field):
        ...
        Example:
            ...
            avg_age = await User.aobjects(active=True).average('age')

            # Average nested field
            avg_rating = await Product.aobjects.average('reviews.rating')
    ...
    def __aiter__(self):
        """Enable async iteration with 'async for'.

        Returns:
            self: The queryset iterator

        Example:
            async for user in User.aobjects(active=True):
                print(user.name)
        """
        return self
    ...
    async def __anext__(self):
        ...
        try:
            raw = await (await self._cursor).__anext__()
        except StopAsyncIteration:
            raise
    ...
    async def rewind(self):
        ...
        self._iter = False
        (await self._cursor).rewind()

    # Properties
    @property
    async def _collection(self):
        """
        Return the AsyncCollection for this queryset, considering:
    ...
    @property
    async def _cursor(self, max_depth=1):
        """Get or create the async MongoDB cursor for this queryset.
    ...
    async def _item_frequencies_map_reduce(self, field, normalize=False):
        ...
        """
        values = await self.map_reduce(map_func, reduce_func, "inline")
        frequencies = {}
        for f in values:
            key = f.key
    ...
    def _chainable_method(self, method_name, val):
        ...
        - Sync version calls method on cursor immediately
