...
class TopLevelDocumentMetaclass(DocumentMetaclass):
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        new_class._auto_id_field = getattr(parent_doc_cls, "_auto_id_field", False)
    ...
    def get_auto_id_names(mcs, new_class):
        ...
        if id_name not in new_class._fields and id_db_name not in (
            v.db_field for v in new_class._fields.values()
        ):
            return id_name, id_db_name
        id_basename, id_db_basename, i = "auto_id", "_auto_id", 0
        while id_name in new_class._fields or id_db_name in (
            v.db_field for v in new_class._fields.values()
        ):
