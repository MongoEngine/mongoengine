...
class TestInheritance(MongoDBAsyncTestCase):
    ...
    async def test_constructor_cls(self):
        ...
        await test_doc.asave()
        saved_doc = await DataDoc.aobjects.with_id(test_doc.id)
        assert test_doc._cls == saved_doc._cls
        assert test_doc.embed._cls == saved_doc.embed._cls
        await test_doc.adelete()
    ...
    async def test_polymorphic_queries(self):
        ...
        classes = [obj.__class__ async for obj in Animal.aobjects]
        assert classes == [Animal, Fish, Mammal, Dog, Human]

        classes = [obj.__class__ async for obj in Mammal.aobjects]
        assert classes == [Mammal, Dog, Human]

        classes = [obj.__class__ async for obj in Human.aobjects]
    ...
    async def test_allow_inheritance(self):
        ...
        dog = await Animal(name="dog").asave()
    ...
    async def test_using_abstract_class_in_reference_field(self):
        ...
        dad = await Dad(name="5").asave()
        await Home(dad=dad, address="street").asave()

        home = await Home.aobjects.first()
        home.address = "garbage"
        await home.asave()  # Was failing with ValidationError
    ...
    async def test_abstract_class_referencing_self(self):
        ...
        user = await User(name="John").asave()
        user2 = await User(name="Foo", creator=user).asave()

        user2 = await User.aobjects.with_id(user2.id)
        user2.name = "Bar"
        await user2.asave()  # Was failing with ValidationError
    ...
    async def test_inherited_collections(self):
        ...
        class Drinker(Document):
            drink = GenericReferenceField(choices=(Drink,))
        ...
        await Drink.adrop_collection()
        await AlcoholicDrink.adrop_collection()
        await Drinker.adrop_collection()

        red_bull = Drink(name="Red Bull")
        await red_bull.asave()

        programmer = Drinker(drink=red_bull)
        await programmer.asave()

        beer = AlcoholicDrink(name="Beer")
        await beer.asave()
        real_person = Drinker(drink=beer)
        await real_person.asave()
        drinks = await Drinker.aobjects.select_related("drink").to_list()
        assert drinks[0].drink.name == red_bull.name
        assert drinks[1].drink.name == beer.name
