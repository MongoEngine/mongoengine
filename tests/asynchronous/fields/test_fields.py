...
class TestField(MongoDBAsyncTestCase):
    ...
    async def test_default_values_nothing_set(self):
    ...
    async def test_custom_field_validation_raise_validation_error(self):
        ...
        await Person(name="garbage").asave()

    async def test_default_values_set_to_None(self):
    ...
    async def test_default_values_when_setting_to_None(self):
        """Ensure that default field values are used when creating
        a document.
        """
    ...
    async def test_default_value_is_not_used_when_changing_value_to_empty_list_for_strict_doc(
        ...
        doc = await Doc(x=[1]).asave()
        doc.x = []
        await doc.asave()
        reloaded = await Doc.aobjects.get(id=doc.id)
        assert reloaded.x == []
    ...
    async def test_default_values_when_deleting_value(self):
    ...
    async def test_required_values(self):
    ...
    async def test_not_required_handles_none_in_update(self):
        ...
        await HandleNoneFields.adrop_collection()
        ...
        ret = (await HandleNoneFields.aobjects.all().to_list())[0]
    ...
    async def test_not_required_handles_none_from_database(self):
        ...
        await (await HandleNoneFields._aget_collection()).update_one(
            {"_id": doc.id},
            {"$unset": {"str_fld": 1, "int_fld": 1, "flt_fld": 1, "comp_dt_fld": 1}},
        )

        # Retrieve data from db and verify it.
        ret = await HandleNoneFields.aobjects.first()
        assert ret.str_fld is None
        assert ret.int_fld is None
        assert ret.flt_fld is None
        assert ret.comp_dt_fld is None
    ...
    async def test_default_id_validation_as_objectid(self):
    ...
    async def test_db_field_validation(self):
    ...
    async def test_list_validation(self):
        ...
        class BlogPost(Document):
            ...
            generic = ListField(GenericReferenceField(choices=(User,)))
    ...
    async def test_reverse_list_sorting(self):
        ...
        await CategoryList.adrop_collection()
    ...
    async def test_list_field(self):
        ...
        assert await BlogPost.aobjects.count() == 3
        assert await BlogPost.aobjects.filter(info__exact="test").count() == 1
        assert await BlogPost.aobjects.filter(info__0__test="test").count() == 1
        ...
        post.info = ["1", "2"]
        await post.asave()
        post = await BlogPost.aobjects(info=["1", "2"]).get()
        post.info += ["3", "4"]
        await post.asave()
        assert await BlogPost.aobjects(info=["1", "2", "3", "4"]).count() == 1
        post = await BlogPost.aobjects(info=["1", "2", "3", "4"]).get()
        post.info *= 2
        await post.asave()
        assert (
    ...
    async def test_list_field_lexicographic_operators(self):
        ...
        await blogSmall.asave()
        await blogSmall.areload()

        blogLargeA = BlogPost(ref="big")
        blogLargeA.text_info = ["a", "z", "j"]
        blogLargeA.bool_info = [False, True]
        await blogLargeA.asave()
        await blogLargeA.areload()
        ...
        await blogLargeB.asave()
        await blogLargeB.areload()
    ...
    async def test_list_field_max_length_set_operator(self):
        ...
        foo = await Foo.aobjects.create(items=[1, 2, 3])
        with pytest.raises(ValidationError) as exc_info:
            await foo.amodify(set__items=[1, 2, 3, 4, 5])
        assert "List is too long" in str(exc_info.value)
    ...
    async def test_slice_marks_field_as_changed(self):
        ...
        simple = await Simple(widgets=[1, 2, 3, 4]).asave()
        simple.widgets[:3] = []
        assert ["widgets"] == simple._changed_fields
        await simple.asave()

        simple = await simple.areload()
    ...
    async def test_list_field_complex(self):
        ...
        # Test querying
        assert await Simple.aobjects.filter(mapping__1__value=42).count() == 1
    ...
    async def test_embedded_db_field(self):
        ...
        await Test.aobjects.update_one(inc__embedded__number=1)
    ...
    async def test_double_embedded_db_field(self):
    ...
    async def test_double_embedded_db_field_from_son(self):
        """Make sure multiple layers of embedded docs resolve db fields
        from SON properly.
        """
    ...
    async def test_embedded_document_inheritance(self):
        ...
        assert 47 == (await BlogPost.aobjects.first()).author.power
    ...
    async def test_embedded_document_inheritance_with_list(self):
        ...
        assert content == (await User.aobjects.first()).groups[0].content
    ...
    async def test_reference_miss(self):
        ...
        class Bar(Document):
            ...
            generic_ref = GenericReferenceField(choices=(Foo,))

        await Foo.adrop_collection()
        await Bar.adrop_collection()

        foo = await Foo().asave()
        bar = await Bar(ref=foo, generic_ref=foo).asave()

        # Reference is no longer valid
        await foo.adelete()

        bar = await Bar.aobjects.select_related("ref", "generic_ref").get()
        ...
        bar = await Bar.aobjects.get()
        expected = foo.to_dbref()
        assert bar.ref == expected
        assert bar.generic_ref.value == {"_ref": expected, "_cls": "Foo"}
    ...
    async def test_recursive_reference(self):
        ...
        peter = await Employee.aobjects.with_id(peter.id)
    ...
    async def test_recursive_embedding(self):
        ...
        tree = await Tree.aobjects.first()
    ...
    async def test_drop_abstract_document(self):
        ...
        with pytest.raises(OperationError):
            await AbstractDoc.adrop_collection()
    ...
    async def test_dynamic_fields_embedded_class(self):
        ...
        await Doc(my_id=1, embed_me=Embed(field_1="hello"), field_x="x").asave()
    ...
    async def test_dynamicfield_dump_document(self):
        ...
        to_embed = await ToEmbed(id=2, recursive=to_embed_recursive).asave()
        ...
        to_embed_child = await ToEmbedChild(id=2, recursive=to_embed_recursive).asave()
...
class TestEmbeddedDocumentListField(MongoDBAsyncTestCase):
    ...
    async def test_fails_upon_validate_if_provide_a_doc_instead_of_a_list_of_doc(self):
    ...
    async def test_chained_filter(self):
        """
    ...
    async def test_chained_filter_exclude(self):
        """
    ...
    async def test_count(self):
        ...
        """
        assert len(self.post1.comments) == 2
        assert len(self.post1.comments) == len(self.post1.comments)
    ...
    async def test_single_keyword_update(self):
        ...
        comments = (await self.BlogPost.aobjects(id=self.post1.id).first()).comments
    ...
    async def test_delete(self):
        ...
        # database.
        assert (await self.BlogPost.aobjects(id=self.post1.id).first()).comments == []
    ...
    async def test_custom_data(self):
        ...
        await CustomData.adrop_collection()

        a1 = await CustomData(a_field=1, c_field=2).asave()
        assert 2 == a1.c_field
...
class TestUtils(MongoDBAsyncTestCase):
    async def test__no_dereference_for_fields(self):
        ...
        # Test instance fetched outside context mgr, patch on the instance field, there is no effect on this
        group = await Group.aobjects.select_related("member").first()
        assert isinstance(group.member, User)
