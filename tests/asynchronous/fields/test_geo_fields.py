...
class TestGeoField(MongoDBAsyncTestCase):
    ...
    async def test_geo_indexes_recursion(self):
        ...
        await Parent(name="Berlin").asave()
        info = await (await Parent._aget_collection()).index_information()
        assert "location_2d" not in info
        info = await (await Location._aget_collection()).index_information()
