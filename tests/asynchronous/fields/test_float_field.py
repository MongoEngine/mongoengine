...
class TestFloatField(MongoDBAsyncTestCase):
    async def test_float_ne_operator(self):
        ...
        await TestDocument(float_fld=None).asave()
        await TestDocument(float_fld=1).asave()

        assert 1 == await TestDocument.aobjects(float_fld__ne=None).count()
        assert 1 == await TestDocument.aobjects(float_fld__ne=1).count()
    ...
    async def test_query_none_value_dont_raise(self):
        ...
        _ = await BigPerson.aobjects(height=None).to_list()
