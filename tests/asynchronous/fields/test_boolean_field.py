...
class TestBooleanField(MongoDBAsyncTestCase):
    async def test_storage(self):
        ...
        await person.asave()
        assert await async_get_as_pymongo(person) == {"_id": person.id, "admin": True}

    async def test_construction_does_not_fail_uncastable_value(self):
        class BoolFail:
            def __bool__(self):
                return "bogus"
    ...
    async def test_weirdness_constructor(self):
