...
class TestEmbeddedDocumentField(MongoDBAsyncTestCase):
    async def test___init___(self):
    ...
    async def test___init___throw_error_if_document_type_is_not_EmbeddedDocument(self):
        with pytest.raises(ValidationError):
            EmbeddedDocumentField(dict)

    async def test_document_type_throw_error_if_not_EmbeddedDocument_subclass(self):
    ...
    async def test_embedded_document_list_field__has__instance_weakref(self):
        ...
        posts = await Post.aobjects.to_list()
        for post in posts:
    ...
    async def test_embedded_document_field_validate_subclass(self):
    ...
    async def test_query_embedded_document_attribute(self):
        ...
        with pytest.raises(InvalidQueryError) as exc_info:
            await Person.aobjects(settings__notexist="bar").first()
        assert str(exc_info.value) == 'Cannot resolve field "notexist"'

        with pytest.raises(LookUpError):
            Person.aobjects.only("settings.notexist")

        # Test existing attribute
        assert (await Person.aobjects(settings__foo1="bar1").first()).id == p.id
        only_p = await Person.aobjects.only("settings.foo1").first()
        assert only_p.settings.foo1 == p.settings.foo1
        assert only_p.settings.foo2 is None
        assert only_p.name is None

        exclude_p = await Person.aobjects.exclude("settings.foo1").first()
    ...
    async def test_query_embedded_document_attribute_with_inheritance(self):
        ...
        # Test non exiting attribute
        with pytest.raises(InvalidQueryError) as exc_info:
            assert (await Person.aobjects(settings__notexist="bar").first()).id == p.id
        assert str(exc_info.value) == 'Cannot resolve field "notexist"'
    ...
    async def test_query_list_embedded_document_with_inheritance(self):
        ...
        assert await Record.aobjects(posts__title="foo").count() == 2


class TestGenericEmbeddedDocumentField(MongoDBAsyncTestCase):
    ...
    async def test_query_generic_embedded_document_attribute(self):
        ...
        p1 = await Person(settings=AdminSettings(foo1="bar1")).asave()
        p2 = await Person(settings=NonAdminSettings(foo2="bar2")).asave()
        ...
        # Test existing attribute
        assert (await Person.aobjects(settings__foo1="bar1").first()).id == p1.id
        assert (await Person.aobjects(settings__foo2="bar2").first()).id == p2.id
    ...
    async def test_deepcopy_set__instance(self):
