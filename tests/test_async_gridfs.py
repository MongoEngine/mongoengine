...
class TestAsyncGridFS:
    ...
    async def setup_and_teardown(self):
        ...
        # Setup
        await connect_async(
            db="mongoenginetest_async_gridfs", alias="async_gridfs_test"
        )
    ...
    async def test_async_file_upload(self):
        ...
        proxy = await AsyncFileDoc.file.async_put(
            file_obj, instance=doc, filename="test.txt"
        )
    ...
    async def test_async_file_read(self):
        ...
        await AsyncFileDoc.file.async_put(
            file_obj, instance=doc, filename="read_test.txt"
        )
        await doc.async_save()
    ...
    async def test_async_file_delete(self):
        ...
        new_proxy = AsyncGridFSProxy(
            grid_id=grid_id, db_alias="async_gridfs_test", collection_name="fs"
        )
    ...
    async def test_async_file_replace(self):
        ...
        proxy = await AsyncFileDoc.file.async_put(
            file_obj, instance=doc, filename="initial.txt"
        )
    ...
    async def test_custom_collection_name(self):
        ...
        proxy = await AsyncImageDoc.image.async_put(
            file_obj, instance=doc, filename="image.jpg"
        )
        await doc.async_save()
    ...
    async def test_multiple_files(self):
        ...
        for i in range(3):
            ...
            proxy = await AsyncFileDoc.file.async_put(
                file_obj, instance=doc, filename=f"file{i}.txt"
            )
            await doc.async_save()
