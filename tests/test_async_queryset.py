...
class TestAsyncQuerySet:
    ...
    async def test_async_get(self):
        ...
        result = await AsyncAuthor.objects.async_get(
            Q(name="Unique Author") & Q(age=35)
        )
    ...
    async def test_async_count(self):
        ...
        limited_count = await AsyncAuthor.objects.limit(3).async_count(
            with_limit_and_skip=True
        )
        assert limited_count == 3

        # Test count with skip
        skip_count = await AsyncAuthor.objects.skip(2).async_count(
            with_limit_and_skip=True
        )
        assert skip_count == 3
    ...
    async def test_async_create(self):
        ...
        author = await AsyncAuthor.objects.async_create(name="Created Author", age=45)
    ...
    async def test_async_update(self):
        ...
        # Create some authors
        for i in range(5):
            await AsyncAuthor.objects.async_create(name=f"Author {i}", age=20 + i)
        ...
        updated = await AsyncAuthor.objects.filter(age=50).async_update(inc__age=5)
        assert updated == 2
    ...
    async def test_async_update_one(self):
        ...
        # Create some authors
        for i in range(3):
            await AsyncAuthor.objects.async_create(name=f"Author {i}", age=30)
    ...
    async def test_queryset_chaining(self):
        ...
        # Create test data
        for i in range(10):
            await AsyncAuthor.objects.async_create(name=f"Author {i}", age=20 + i * 2)

        # Test complex chaining
        result = (
            await AsyncAuthor.objects.filter(age__gte=25)
            .filter(age__lte=35)
            .order_by("-age")
            .limit(3)
            .async_to_list()
    ...
    async def test_async_with_references(self):
        ...
        # Create an author
        author = await AsyncAuthor.objects.async_create(name="Book Author", age=40)
    ...
    async def test_as_pymongo(self):
        ...
        # Create a document
        await AsyncAuthor.objects.async_create(name="PyMongo Test", age=30)
