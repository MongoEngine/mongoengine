...
class TestAsyncAggregation:
    ...
    async def test_async_distinct_basic(self):
        ...
        try:
            # Create test data
            await Product(
                name="Widget A", category="widgets", tags=["new", "hot"]
            ).async_save()
            await Product(
                name="Widget B", category="widgets", tags=["sale"]
            ).async_save()
            await Product(
                name="Gadget A", category="gadgets", tags=["new"]
            ).async_save()
            await Product(
                name="Gadget B", category="gadgets", tags=["hot", "sale"]
            ).async_save()
    ...
    async def test_async_distinct_embedded(self):
        ...
        try:
            ...
            await Customer(
                name="Bob", address=Address(city="London", country="UK")
            ).async_save()

            await Customer(
                name="Charlie", address=Address(city="New York", country="USA")
            ).async_save()

            await Customer(
                name="David", address=Address(city="Paris", country="France")
            ).async_save()
            ...
            usa_cities = await Customer.objects.filter(
                address__country="USA"
            ).async_distinct("address.city")
    ...
    async def test_async_distinct_no_results(self):
        ...
        try:
            ...
            colors = await Item.objects.filter(name="NonExistent").async_distinct(
                "color"
            )
            assert colors == []
