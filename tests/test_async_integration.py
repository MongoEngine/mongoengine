...
class TestAsyncIntegration:
    ...
    async def test_concurrent_operations(self):
        ...
        await asyncio.gather(*[user.async_save() for user in users])
        ...
        await asyncio.gather(*[user.async_reload() for user in users])

        for i, user in enumerate(users):
            assert user.age == 21 + i

        # Delete all users concurrently
        await asyncio.gather(*[user.async_delete() for user in users])
