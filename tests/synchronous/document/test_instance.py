...
from tests.synchronous.fixtures import PickleSignalsTest
from tests.synchronous.utils import (
    MongoDBTestCase,
    db_ops_tracker,
    get_as_pymongo,
...
class TestDocumentInstance(MongoDBTestCase):
    ...
    def test_reference_inheritance(self):
        ...
        assert list_stats == CompareStats.objects.select_related("stats").first().stats
    ...
    def test_reload_sharded(self):
        ...
        with query_counter() as q:
            doc.reload()
            query_op = q.db.system.profile.find({"ns": f"{MONGO_TEST_DB}.animal"})[0]
    ...
    def test_reload_sharded_with_db_field(self):
        ...
        with query_counter() as q:
            doc.reload()
            query_op = q.db.system.profile.find({"ns": f"{MONGO_TEST_DB}.person"})[0]
    ...
    def test_save_cascades(self):
        ...
        p = Person.objects(name="Wilson Jr").select_related("parent").get()
    ...
    def test_can_save_if_not_included(self):
        ...
        class Doc(Document):
            ...
            generic_reference_field = GenericReferenceField(
                default=lambda: Simple().save(), choices=(Simple,)
            )
    ...
    def test_query_count_when_saving(self):
        ...
        user = User.objects.select_related("orgs").first()
        ...
        with query_counter() as q:
            assert q == 0
            sub = UserSubscription.objects.select_related("user").first()
            assert q == 1
            sub.user.name = "Test"
            assert q == 1
        ...
        with query_counter() as q:
            ...
            assert q == 1
            sub.save(cascade=True)
            assert q == 3  # One for the UserSub and one for the User
    ...
    def test_save_reference(self):
        ...
        post_obj = BlogPost.objects.select_related("author").first()
    ...
    def test_reverse_delete_rule_cascade_triggers_pre_delete_signal(self):
        ...
        class BlogPost(Document):
            ...
            def pre_delete(cls, sender, document, **kwargs):
                ...
                Editor.objects(pk=document.editor.pk).update(dec__review_queue=1)
    ...
    def test_db_alias_tests(self):
        ...
        register_connection("testdb-2", f"{MONGO_TEST_DB}_3")
        register_connection("testdb-3", f"{MONGO_TEST_DB}_4")
    ...
    def test_db_alias_overrides(self):
        ...
        register_connection("testdb-2", f"{MONGO_TEST_DB}_2")
        ...
        assert MONGO_TEST_DB == A._get_collection().database.name
        assert f"{MONGO_TEST_DB}_2" == B._get_collection().database.name
    ...
    def test_switch_db_multiple_documents_same_context(self):
        register_connection("testdb-1", f"{MONGO_TEST_DB}_2")
        register_connection("testdb-2", f"{MONGO_TEST_DB}_3")
        ...
        with switch_db(Group, "testdb-1") as Group_1:
            Group_1.drop_collection()

        # --- clean testdb-2 for Post ---
        with switch_db(Post, "testdb-2") as Post_2:
            Post_2.drop_collection()

        # Seed default DB
        Group(name="group-default").save()
        ...
        with switch_db(Group, "testdb-1"), switch_db(Post, "testdb-2"):
            Group(name="group-testdb-1").save()
            Post(title="post-testdb-2").save()
        ...
        with switch_db(Group, "testdb-1"):
            g1 = Group.objects.first()
            assert g1.name == "group-testdb-1"

        with switch_db(Post, "testdb-2"):
            p2 = Post.objects.first()
            assert p2.title == "post-testdb-2"

    def test_switch_db_and_switch_collection_instance(self):
        register_connection("testdb-1", f"{MONGO_TEST_DB}_2")
        ...
        Group.drop_collection()
        with switch_db(Group, "testdb-1"):
            with switch_collection(Group, "group_alt"):
                Group.drop_collection()
        ...
        g0.update(set__name="hello - update")
    ...
    def test_switch_multiple_db_and_multiple_collection_same_time(self):
        register_connection("testdb-a", f"{MONGO_TEST_DB}_2")
        register_connection("testdb-b", f"{MONGO_TEST_DB}_2")
    ...
    def test_accessing_objects_with_indexes_error(self):
        ...
        # Ensure the index creation exception isn't swallowed (#1688) #todo
